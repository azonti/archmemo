cryptsetup luksFormat --cipher aes-xts-plain64 --key-size 512 --hash sha512 --iter-time 5000 --use-random --label luks /dev/sda3
cryptsetup luksOpen /dev/sda3 btrfs
...
pacstrap /mnt base base-devel linux linux-firmware dosfstools btrfs-progs linux-headers v4l2loopback-dkms bcwc-pcie-git vim man-db man-pages
...
EOF
chmod a+x /usr/lib/systemd/system-sleep/usb-suspend-workaround.sh
...
## 個人設定/外観/アイコン
## 個人設定/外観/フォント/レンダリング
## 個人設定/外観/設定/ウィンドウ拡大縮小
## ハードウェア/ディスプレイ/全般/2/スケール
## ハードウェア/電源管理/一般/ボタン
## ハードウェア/電源管理/システム
## ハードウェア/電源管理/ディスプレイ
## ハードウェア/電源管理/セキュリティ
